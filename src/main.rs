...
fn duration_multiplier(intervals: u32) -> f32 {
    if intervals <= 180 * 60 / INTERVAL_LEN_SECS {
        ...
        1.0
    } else {  // intervals <= 480 * 60 / INTERVAL_LEN_SECS
        0.5
    }
}
...
impl Score {
    ...
    fn insert(&mut self, timestamp: u32, keypresses: u8) {
    ...
    fn calculate(&self) -> f32 {
        ...
        for (timestamp, keypresses) in self.series.iter() {
            ...
            if recovery_debt >= 60 * 60 / INTERVAL_LEN_SECS { // 1 hour
                num_lack_of_recovery_intervals += 1;
            }
            previous_timestamp = *timestamp;
        }

        let lack_of_recovery_factor = 1.0 - num_lack_of_recovery_intervals as f32 * LACK_OF_RECOVERY_INTERVAL_PENALTY;
        ...
        sum_keypresses as f32 / (total_work_intervals as f32 * lack_of_recovery_factor * duration_multiplier(total_work_intervals))
    }
}
...
fn main() {
    // Read from stdin as binary data. The first 4 bytes are the timestamp / 5, big endian, the next byte is the number of key presses
